...
- **Echo Assistant（Phase 2）：** 对话式 AI 助手，基于 RAG 回答关于用户记忆的问题 ✅（v1.1 已交付）
- **AI 陪伴建议（v1.2）：** 保存记忆后异步生成温情/实用/启发型建议 ✅
...
### 3.3 Phase 2 规划（Echo Assistant）✅ 已交付（v1.1）
...
### 3.5 v1.1 已交付功能（Echo Assistant + 质量提升）

| 模块 | 功能点 | 状态 |
|------|--------|------|
| **Echo Assistant** | Chat 侧边栏组件（展开/收起） | ✅ |
| | RAG 检索：语义搜索 → 相关记忆 → LLM 生成回答 | ✅ |
| | 多轮对话 + 引用来源标注 | ✅ |
| | 对话历史管理 | ✅ |
| **质量提升** | OAuth state 内存泄漏修复 | ✅ |
| | Gateway 健康检查完善 | ✅ |
| | 路径遍历漏洞修复 | ✅ |
| | 双模式分页（cursor / offset） | ✅ |
| | Go 单元测试 55 个全部通过 | ✅ |

### 3.6 v1.2 已交付功能（"记忆的温度"）

| 模块 | 功能点 | 状态 |
|------|--------|------|
| **AI 陪伴建议** | 异步生成温情/实用/启发型建议 | ✅ |
| | 文字/链接不同提示策略 | ✅ |
| | 建议持久化，支持反馈（like/dislike） | ✅ |
| | 默认关闭，高级选项可配置 | ✅ |
| **标签重生** | 首页横向标签栏 + 多选 AND 过滤 | ✅ |
| | `/tags` 管理页（云图/卡片/列表） | ✅ |
| | 标签颜色预设（16 色，按用户存储） | ✅ |
| | 标签合并（AI 检测相似标签） | ✅ |
| | 相关标签（共现统计） | ✅ |
| **记忆的温度** | 记忆 Streaks（连续记录天数） | ✅ |
| | 那年今日（首页展示一年前记忆） | ✅ |
| | 时间胶囊（7/30/100 天封印，独立入口） | ✅ |
| | 每日回顾（可折叠卡片） | ✅ |
