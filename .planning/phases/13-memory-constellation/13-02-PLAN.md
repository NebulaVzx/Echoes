...
must_haves:
  truths:
    - "GET /api/v1/constellation returns graph nodes and edges for authenticated user with pagination support"
...
- **D-01**: 分层加载 — GetConstellation returns first 100 recent + all starred, has_more flag, supports offset for pagination
...
<task type="auto">
  ...
  <read_first>
    - services/memory-service/internal/service/memory_service.go (existing struct and methods)
    - services/memory-service/internal/repository/relation_repository.go (RelationRepository interface)
    - services/memory-service/internal/domain/memory.go (ConstellationResponse, ExploreResponse types)
  </read_first>
  <action>
...
6. Add required imports to memory_service.go if not present:
  ...
  <acceptance_criteria>
    - grep -c "func (s \*MemoryService) GetConstellation" services/memory-service/internal/service/memory_service.go == 1
    - grep -c "func (s \*MemoryService) Explore" services/memory-service/internal/service/memory_service.go == 1
    - grep -c "relationRepo" services/memory-service/internal/service/memory_service.go >= 2
    - grep -c "offset int" services/memory-service/internal/service/memory_service.go >= 1
    - grep -c "GetConstellationNodes.*offset" services/memory-service/internal/service/memory_service.go >= 1
  </acceptance_criteria>
  <verify>
  ...
  <done>MemoryService has GetConstellation (with offset) and Explore methods; relationRepo field added; LLM reason generation with caching implemented; all imports present</done>
</task>
...
<task type="auto">
  ...
  <read_first>
    - services/memory-service/internal/transport/memory_handler.go (existing handler patterns, route registration)
    - services/memory-service/internal/service/memory_service.go (GetConstellation signature with offset)
    - services/memory-service/cmd/main.go (existing repository wiring)
  </read_first>
  <action>
...
4. Ensure `strconv` import is present in memory_handler.go for offset parsing.
  </action>
  <acceptance_criteria>
    - grep -c "GetConstellation" services/memory-service/internal/transport/memory_handler.go >= 1
    - grep -c "Explore" services/memory-service/internal/transport/memory_handler.go >= 1
    - grep -c "/constellation" services/memory-service/internal/transport/memory_handler.go >= 1
    - grep -c "NewGormRelationRepository" services/memory-service/cmd/main.go >= 1
    - grep -c "offset" services/memory-service/internal/transport/memory_handler.go >= 1
    - grep -c "strconv.Atoi" services/memory-service/internal/transport/memory_handler.go >= 1
  </acceptance_criteria>
  <verify>
  ...
  <done>GetConstellation handler accepts offset query param; Explore handler returns flat result structure; routes added before /memories/:id; relationRepo wired in main.go; service compiles</done>
</task>
...
- [ ] GET /api/v1/constellation route registered with offset query param support
- [ ] GET /api/v1/memories/:id/explore route registered
- [ ] MemoryService has GetConstellation (with offset) and Explore methods
...
- GET /api/v1/constellation accepts ?offset=N for pagination
- GET /api/v1/memories/:id/explore returns 5-8 related memories with AI-generated reasons
