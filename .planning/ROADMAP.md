...
| 16 | 用户管理中心 | 记忆 DNA、AI 助手人格、数据主权、学习路径、账户基础 | 8 | 📋 规划中（8 plans / 4 waves） |
...
| Plan | 目标 | 状态 |
|------|------|------|
| 12-01 | 文件上传 + 记忆匣 + 快速模板 + 星标/来源/草稿 + 批量导入 + 智能粘贴识别 | ✅ 已完成（Task 1-9，端到端验证通过） |
...
| Plan | 目标 | Wave | 状态 |
|------|------|------|------|
| 13-01 | 数据库迁移 + Domain 类型 + RelationRepository + Constellation 查询方法（含 offset 分页） | 1 | ✅ 已完成 |
| 13-02 | MemoryService 扩展 + Constellation/Explore Handler（含 offset 查询参数）+ LLM 关联说明 + 依赖注入 | 1 | ✅ 已完成 |
| 13-03 | 安装依赖 + 类型定义（flat ExploreResponse）+ ConstellationGraph（读取 node.color）+ GraphControls + 星座页面（过滤不破坏原数据） | 2 | ✅ 已完成 |
| 13-04 | ExplorePanel + BreadcrumbTrail + RelatedMemoryCard + ConnectionReason + /explore 页面 | 3 | ✅ 已完成 |
| 13-05 | Hooks（无颜色重计算）+ 页面集成 + RightPanel 联动 + 键盘快捷键 | 3 | ✅ 已完成 |
| 13-06 | 端到端验证（人工检查点） | 4 | ⏳ 待人工确认 |

*Roadmap updated: 2026-05-09 — Phase 13 技术实现全部完成，自动验证通过，等待人工 E2E*
...
| Plan | 目标 | Wave | 状态 |
|------|------|------|------|
| 16-01 | 数据库迁移 + User 领域扩展（user_insights 表、settings 扩展、user_sessions 表） | 1 | 📋 待执行 |
| 16-02 | DNA Consumer + 人格 Prompt 模板 + LLM 注入点 | 1 | 📋 待执行 |
| 16-03 | User Service API 扩展（头像上传、密码修改、邮箱变更、账户注销、TOTP） | 2 | 📋 待执行 |
| 16-04 | Memory Service API 扩展（DNA 查询、统计聚合、数据导出端点） | 2 | 📋 待执行 |
| 16-05 | /profile 页面框架 + DNA 可视化（雷达图、统计卡片、洞察语） | 3 | 📋 待执行 |
| 16-06 | AI 人格选择 + 学习路径 + 数据仪表盘 | 3 | 📋 待执行 |
| 16-07 | /settings 调整（隐私与数据、账户安全） | 4 | 📋 待执行 |
| 16-08 | 导航集成 + Command Palette + E2E 验证 | 4 | 📋 待执行 |

*Roadmap updated: 2026-05-14 — Phase 16 规划完成，8 plans / 4 waves，待执行*
